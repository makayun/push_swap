...
/*   Updated: 2025/03/11 19:27:40 by mmakagon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ps_find_smallest2(t_data *data, const int biggest)
{
	int		i;
	int		index;
	long	smallest;
	long	current;
	// const int avg = ps_average(data->pool, data->pool_size);
	...
	while (i < data->b_size)
	{
		...
		current += (biggest - data->pool[i]) + abs(data->pool[i] - data->pool[data->b_size]);
		if (smallest > current)
		{
			smallest = current;
			index = i;
...
int	ps_biggest(const int *pool, const int pool_size)
{
	int	i;
	int biggest;
...
{
	const int	biggest = ps_biggest(pool, pool_size);
	int			smallest;
	int			a_size;

	while (*b_size > 0)
	{
		smallest = ps_find_smallest2(data, biggest);
